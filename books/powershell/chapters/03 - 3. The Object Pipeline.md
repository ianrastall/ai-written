...
Nested pipelines are sometimes the clearest way to express "for each outer item, run a second query and transform that result." The key point for Part I is simple: in ordinary sequential `ForEach-Object` usage, inner script blocks can read variables from the outer scope (and can also modify them, because they run in the caller's scope).

This is different from remoting, background jobs, and `ForEach-Object -Parallel`, where execution crosses a runspace or process boundary and `$using:` (or explicit parameters) is required. In a sequential nested pipeline, a well-named local variable is usually enough.

#### 3.2.5.1 Outer Context in an Inner Pipeline

The outer pipeline often establishes context (for example, "which directory am I in?"), and the inner pipeline uses that context while processing a smaller set of items. The threshold variable below is just caller-defined configuration for this run:

<!-- Illustrative -->
```powershell
# Caller-defined threshold for this run (1 MB here for demonstration)
$sizeThresholdMB = 1

Get-ChildItem -Path $env:TEMP -Directory -ErrorAction SilentlyContinue | ForEach-Object {
    $parentDir = $_

    Get-ChildItem -Path $parentDir.FullName -File -ErrorAction SilentlyContinue |
        Where-Object Length -gt ($sizeThresholdMB * 1MB) |
        Select-Object @{n='ParentDirectory';e={$parentDir.Name}}, Name, Length
}
```

The inner pipeline reads `$parentDir` and `$sizeThresholdMB` directly without extra plumbing. If nested pipelines start to feel like control-flow scaffolding rather than data-flow, extract the inner logic into a helper function and keep the pipeline shape simple.
...
Filtering as early as possible in the pipeline reduces downstream work, improving both speed and memory usage. In practice, "filter left" usually means either pushing the filter into the source command/provider when possible, or keeping the pipeline object-based until after filtering.

Many providers support source-side filtering, which is more efficient than retrieving everything and filtering later:
...
The same idea applies when a later command would force serialization to text. Filter objects first, then serialize the smaller result:
...
The efficient version keeps the pipeline object-based during filtering and only converts to text at the end.
...
#### 3.4.3.2 Write-Host Is Not a Passthrough

If you want host-visible output while continuing the pipeline, remember that `Write-Host` does not pass the input object downstream. Use an explicit passthrough pattern:
...
**3.4.3.2.1 Viewing data while preserving the pipeline:**
...
Get-ChildItem -Path $env:TEMP -OutVariable 'items'
```
...
To see the chapter's concepts working together, imagine you want a quick process report for an interactive troubleshooting session: show long-running processes that have consumed significant CPU, add a memory column in MB, and save the same report text to disk while also viewing it on screen.

Start by defining the thresholds and output path. Using explicit variables keeps the later pipeline readable and makes the intent of each filter obvious:

<!-- Illustrative -->
```powershell
[double]$cpuThresholdSeconds = 50
$startedBefore = (Get-Date).AddMinutes(-30)
$reportPath = Join-Path ([System.IO.Path]::GetTempPath()) 'process_report.txt'

$processes = Get-Process
```

At this point you have live process objects. Keep them as objects while filtering and shaping the data. The pipeline below filters on CPU first, then uses a projection step to add `MemoryMB` and safely capture `StartTime` only for processes that expose it:

<!-- Illustrative -->
```powershell
$reportRows = $processes |
    Where-Object { $null -ne $_.CPU -and $_.CPU -gt $cpuThresholdSeconds } |
    ForEach-Object {
        $proc = $_
        ...
        try {
            ...
            return  # Skip processes whose StartTime is inaccessible
        }

        if ($startTime -lt $startedBefore) {
            [PSCustomObject]@{
...
Notice the order of operations: filter before formatting, keep the pipeline object-based while calculating new properties, and handle a common edge case (`StartTime` access) close to the point where it matters. Only after the result set is finalized do you switch to formatting.

Now build a short summary, render the table to text, and use `Tee-Object` to write the final report to a file while also displaying it:

<!-- Illustrative -->
```powershell
$summaryLines = @(
    "Total Processes: $($processes.Count)"
    "High CPU Processes: $(($processes | Where-Object { $null -ne $_.CPU -and $_.CPU -gt $cpuThresholdSeconds }).Count)"
    "Long-Running Report Rows: $($reportRows.Count)"
    ""
)

$tableText = $reportRows |
    Select-Object Name, Id, Cpu, MemoryMB, StartTime |
    Format-Table -AutoSize |
    Out-String

($summaryLines -join [Environment]::NewLine) + $tableText |
    Tee-Object -FilePath $reportPath
```

This is the core PowerShell pipeline workflow in miniature: retrieve objects, filter and shape them while they are still objects, and format only at the boundary where you need human-readable text. Once that order becomes a habit, your one-liners and scripts become easier to reason about and easier to extend.

### 3.5.1 Key Takeaways

The following table summarizes the most important pipeline habits from this chapter and when to apply them:

| Concept | Key Principle | When to Use |
| --- | --- | --- |
| ByValue / ByPropertyName binding | Cmdlets receive objects through parameter binding, not string parsing | Predicting how a pipeline will connect commands |
| Filter left | Reduce objects as early as possible, preferably at the source | File/provider queries, large datasets, expensive downstream work |
| Projection before formatting | Use `Select-Object` / `ForEach-Object` to shape data, then format at the end | Reports, exports, and reusable pipeline stages |
| `Write-Host` caveat | `Write-Host` does not passthrough objects unless you emit them explicitly | Host-visible progress or inspection during a pipeline |
| Null-aware pipelines | Guard `$null` values and commands that emit no objects | Defensive scripting and reliable automation |
| `Tee-Object` | Split output to a file or variable while preserving the stream | Logging, debugging, and interactive reporting |
