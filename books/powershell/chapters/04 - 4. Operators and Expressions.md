...
Many examples in this chapter use flow-control constructs such as `if`, `elseif`, `foreach`, and `for` to illustrate operator behavior. Chapter 5 ("Flow Control and Logic") covers those constructs in depth; in this chapter, focus on what the expressions and operators are doing inside them.
...
(Get-Process pwsh).CPU / 60 -gt 5 -and ([Environment]::OSVersion.Platform -eq 'Win32NT')
```

This example uses `[Environment]::OSVersion.Platform` instead of `$IsWindows` so it behaves consistently in Windows PowerShell 5.1 and PowerShell 7+.
...
<!-- Tested on: Windows PowerShell 5.1.26100.7920, PowerShell 7.5.4 -->
```powershell
$int32   = 2147483647          # [int]
$int64   = 2147483648L         # [long]
$decimal = 10.1d               # [decimal]

($int32  + 1).GetType().Name   # Double
([int]$int32 + 1).GetType().Name # Double
($int64  + 1).GetType().Name   # Int64
($decimal + 0.2).GetType().Name # Decimal
```

> [!VERSION]
> Verified on Windows PowerShell 5.1 and PowerShell 7.5: these expressions promote to `Double` rather than throwing `OverflowException`.

If you need explicit integer behavior across editions, widen before the operation:

<!-- Illustrative -->
```powershell
([long]$int32 + 1).GetType().Name  # Int64
[int]([int]::MaxValue + 1)         # Fails conversion (value too large for Int32)
```
...
The final line uses the format operator (`-f`) to insert `$total` into a composite format string; Section 4.1.6 introduces string-formatting and string-transformation operators.
...
each `+=` creates a new array and copies the existing elements before appending the new value, so repeated use in loops becomes expensive.
...
For the memory-allocation walkthrough and the O(N²) analysis, see Chapter 2, Section 2.3.2 ("Array Operators (+, +=) and Performance Implications"), especially Section 2.3.2.1 ("The += Antipattern"). The key point here is simply that compound assignment on arrays is convenient but can be surprisingly expensive.
...
The following table summarizes the relative precedence of the operators discussed in this chapter, from highest to lowest. Operators on the same row are grouped at the same precedence level; when in doubt, add parentheses to make evaluation order explicit.

| Level | Category                           | Examples                                                       |
|-------|------------------------------------|----------------------------------------------------------------|
| 1     | Primary / grouping / member access | `()`, `$()`, `@()`, `.`, `::`                                  |
| 2     | Unary operators                    | `-`, `+` (sign), `-not`                                        |
| 3     | Format                             | `-f`                                                           |
| 4     | Multiplicative                     | `*`, `/`, `%`                                                  |
| 5     | Additive                           | `+`, `-`                                                       |
| 6     | Comparison / pattern / string      | `-eq`, `-gt`, `-like`, `-match`, `-replace`, `-split`, `-join`, `-is` |
| 7     | Bitwise / shift                    | `-band`, `-bor`, `-bxor`, `-bnot`, `-shl`, `-shr`             |
...
PowerShell also has operator-specific nuances (for example, unary forms of `-split` and `-join`), so use parentheses whenever readability matters more than memorizing the exact precedence rules.
...
PowerShell also provides the shift operators `-shl` (left shift) and `-shr` (right shift). They are commonly used in the same low‑level scenarios as bitwise operators; for example, `1 -shl 8` yields `256`.
...
**CIDR block math:** Classless Inter‑Domain Routing (CIDR) notation expresses the size of a subnet as a prefix length, such as `/24`. You can use the left-shift operator (`-shl`) to reason about how many host addresses are available:
...
### 4.1.6 String Operators (-f, -replace, -split, -join)

PowerShell includes several operators for formatting and transforming strings without having to call .NET methods directly. These are especially common in scripts that build log messages, parse delimited text, or normalize user input.

- `-f` formats a string using .NET composite formatting placeholders.
- `-replace` replaces text using a regular-expression pattern.
- `-split` splits a string into an array (using a regular-expression delimiter by default).
- `-join` joins an array or collection into a single string.

<!-- Illustrative -->
```powershell
"Total memory: {0:N0} bytes" -f 12345678
'server01.contoso.com' -split '\.'
'a,b,c' -replace ',', ';'
'red','green','blue' -join ' | '
```

`-replace` and `-split` use regular expressions. This chapter keeps those examples simple; Chapter 8 covers regular-expression syntax and escaping in depth.

`-join` and `-split` can also be used in unary form, but the binary forms shown here are the most common in everyday expressions and pipelines.
...
`-match` and `-cmatch` use regular expressions. The example below intentionally uses only simple character classes (`[A-Z]`, `[a-z]`, `\d`); Chapter 8 introduces regular expressions in depth.
...
Like the equality operators, containment operators also support the `c`/`i` casing convention for explicit case handling (for example, `-cin`, `-ccontains`, `-iin`, and `-icontains`). For example, `'alice' -cin @('Alice','Bob')` is `$false`, while `'Alice' -cin @('Alice','Bob')` is `$true`.
...
#### 4.2.5.3 Safe Conversion with -as

The `-as` operator attempts a type conversion and returns `$null` if the conversion cannot be performed. This makes it a safer alternative to a hard cast (`[Type]`) when invalid input is expected and you want to branch instead of throwing.

**Non-throwing conversion with null check:** Use an explicit `$null` check rather than relying on truthiness, because valid conversions can yield values like `0` or `$false`:
...
if ($null -ne $parsed) {
    ...
    Write-Warning "Could not convert '$raw' to [int]."
}
```
...
function Test-Expensive {
    ...
    Write-Verbose "Expensive call ran"
    return $true
}
...
if ($flag -and (Test-Expensive -Verbose)) {
    "Won't run"
}
```
...
if (
    ...
    ($retries -lt 3) -and
    -not $IsPreview   # user-defined flag
) {
...
> [!VERSION]
> Windows PowerShell 5.1 defaults redirection to UTF‑16 LE. PowerShell 7+ defaults to UTF‑8 (no BOM).
...
Ordering matters when you merge and redirect streams because PowerShell processes redirection left to right (just like Bash). For example, `2>&1 > file.txt` first points stream 2 at stream 1's current destination (the console), then redirects stream 1 to the file, so success output goes to `file.txt` while errors still go to the console. Reversing the order (`> file.txt 2>&1`) redirects success first and then merges errors into that file destination, so both streams end up in `file.txt`.
...
In PowerShell 5+, `Write-Host` also writes to the Information stream (6), which means it can be redirected or suppressed with `6>` and `$InformationPreference` (unlike `Write-Output`, which writes to stream 1).
...
| Log success + error output                | `Some-Command >> log.txt 2>&1`                                   |
