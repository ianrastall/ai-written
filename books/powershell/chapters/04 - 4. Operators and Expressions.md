...
($int32  + 1).GetType().Name   # Int64 in PowerShell 7+ (Windows PowerShell 5.1 may throw)
([int]$int32 + 1)              # Throws OverflowException in PowerShell 7+; behavior may vary in 5.1
($int64  + 1).GetType().Name   # Int64
($decimal + 0.2).GetType().Name # Decimal
```

Windows PowerShell 5.1 may throw `OverflowException` here instead of auto‑widening to `Int64`. If you need cross‑version behavior, widen explicitly before the operation:

<!-- Illustrative -->
```powershell
([long]$int32 + 1)   # 2147483648 in both Windows PowerShell 5.1 and PowerShell 7+
```
